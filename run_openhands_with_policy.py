"""Launch OpenHands connected to PECA's independent policy-advisor MCP service."""
...
def main():
    ...
    if not workspace.is_dir() or not python.is_file() or not openhands:
        parser.error("Need an existing workspace, PECA .venv with policy-advisor installed, and OpenHands")
    ...
    compatibility = ROOT / "policy-advisor-mcp/integrations/openhands/compat_entrypoint.py"
    with socket.socket() as sock:
        sock.bind(("127.0.0.1", 0))
        port = sock.getsockname()[1]
    state = (options.state_dir or ROOT / ".artifacts/openhands-policy").expanduser().resolve()
    state.mkdir(parents=True, exist_ok=True, mode=0o700)
    config = {"mcpServers": {"policy-advisor": {
    ...
    with (state / "selector-server.log").open("w") as log:
        ...
        try:
            ...
            while True:
                ...
                try:
                    ...
                    if time.monotonic() >= deadline:
                        raise RuntimeError("Timed out waiting for policy-advisor")
                    time.sleep(0.1)
            ...
            print(f"policy-advisor ready; OpenHands workspace: {workspace}", flush=True)
