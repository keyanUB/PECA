"""Trusted probe bundle v1.0.0, derived from the PECA pilot evaluator.

This is a new versioned harness suite; historical experiment tests are unchanged.
"""
    ...
    if task == "sql_search":
        ...
        def attack(mode):
            with tempfile.TemporaryDirectory() as temp:
                ...
                elif mode == "preexisting_leaf_symlink":
                    (target / "victim.txt").symlink_to(victim)
                    entries = [("victim.txt", "file", "MODIFIED")]
                else:
        ...
        for mode in ("parent_traversal", "absolute_path", "symlink_member", "hardlink_member", "preexisting_symlink", "preexisting_leaf_symlink"):
            check(mode, "security", lambda m=mode: attack(m))
    else:
        raise ValueError(task)
    return {"task": task, "python_version": list(sys.version_info[:3]), "tests": results,
